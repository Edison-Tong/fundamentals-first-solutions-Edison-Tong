...
for (let i = 0; i < people.length; i++) {
  if (people[i].shoeSize) {
    console.log(`${people[i].name} has a shoe size of ${people[i].shoeSize}.`);
  }
}
...
for (let i = 0; i < people.length; i++) {
  if (people[i].phoneNumber) {
    console.log(`${people[i].name}'s phone number is ${people[i].phoneNumber}.`);
  }
}
...
function farthestDriver(drivers) {
  ...
  for (let i = 1; i < drivers.length; i++) {
    if (drivers[i].feetFromUser > farthest.feetFromUser) {
      farthest = drivers[i];
    }
  }
